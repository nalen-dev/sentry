...
type SettingTab = 'time' | 'users' | 'database' | 'threshold' | 'advanced';
...
export default function SettingPage() {
  ...
  const handleSaveAll = async () => {
    if (!isTauri) {
      alert("Settings cannot be saved in a normal web browser. Please run the Tauri desktop app.");
      return;
    }
    try {
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
            ...
            {activeTab === 'database' && (
              <div className="max-w-2xl space-y-6">
                ...
                <div className="pt-4">
                  <button className="px-4 py-2 bg-bg-surface border border-border rounded-lg text-text-primary hover:text-scada-primary transition-colors text-sm font-bold flex items-center">
                    <RotateCcw size={16} className="mr-2" /> Test Connection
                  </button>
                </div>
              </div>
            )}
