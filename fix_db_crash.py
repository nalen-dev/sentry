...
# Replace all `.execute(&pool).await?;` in the migration block with `.execute(&pool).await.ok();`
# But only for the new migration code.

old_migration = """    // FORCE MIGRATION: Update old map_calibration group names to the new standard
...
new_migration = """    // FORCE MIGRATION: Update old map_calibration group names to the new standard (ignore errors if they fail due to uniqueness constraints)
